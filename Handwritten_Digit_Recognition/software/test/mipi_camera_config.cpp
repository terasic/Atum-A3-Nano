...
alt_u8 OV8865_read_cmos_sensor_8(alt_u16 Addr){
    alt_u8 Value;
	i2c_mipi_camera.ReadReg8(Addr, &Value);
...
void OV8865_write_cmos_sensor_8(alt_u16 Addr, alt_u8 Value){
	i2c_mipi_camera.WriteReg8(Addr, Value);
}


void OV8865_write_AF(alt_u8 msb, alt_u8 lsb){
	// VCM149C
	alt_u8 buf[2] = { msb, lsb };
	i2c_mipi_camera.VCMWrite(buf, 2);
}

void OV8865_read_AF(){
	// VCM149C
	alt_u8 buf[2];
	if (!i2c_mipi_camera.VCMRead(buf, 2)) {
		printf("VCM read error (NACK?)\n");
		return;
	}
...
void OV8865_FOCUS_Move_to(alt_u16 a_u2MovePosition)
{
	...
	const alt_u8 device_address = MIPI_AF_I2C_ADDR >> 1;
	bSuccess = i2c_mipi_camera.Open("/dev/i2c_camera",device_address);
	if (!bSuccess)
	...
	// Release I2C bus , due to two I2C master shared!
	i2c_mipi_camera.Close();
...
void MIPI_BIN_LEVEL(alt_u8 level){
	...
	if(level >= 3) level = 3;
	  ...
	  int bSuccess;
	    bSuccess = i2c_mipi_camera.Open("/dev/i2c_camera",MIPI_I2C_ADDR >> 1);
		if (!bSuccess)
			printf("failed to init MIPI- Camera i2c\r\n");
