...
/*
	...
	source_eop,
*/
  // AXI Stream Agent (sink, input)
	s_axi_tdata,
	s_axi_tvalid,
	s_axi_tready,
	s_axi_tuser,    // SOP (start of packet)
	s_axi_tlast,    // EOP (end of packet)

  // AXI Stream Manager (source, output)
	m_axi_tdata,
	m_axi_tvalid,
	m_axi_tready,
	m_axi_tuser,    // SOP
...
// AXI Stream Agent (sink, input)
input  [23:0] s_axi_tdata;
input         s_axi_tvalid;
output        s_axi_tready;
input  [2:0]  s_axi_tuser;    // SOP (start of packet)
input         s_axi_tlast;    // EOP (end of packet)

// AXI Stream Manager (source, output)
output [23:0] m_axi_tdata;
output        m_axi_tvalid;
input         m_axi_tready;
output [2:0]  m_axi_tuser;    // SOP
output        m_axi_tlast;    // EOP
...
*/
// connnect sink & source direct
assign m_axi_tdata  =  (vcm_en & focus_mode & focus_window_border )?{10'h1ff, 10'h1ff, 10'h0}:s_axi_tdata;
...
begin
	...
	else if(s_axi_tvalid & m_axi_tready) begin
...
wire sop/*synthesis keep*/; // start of packet
wire eop/*synthesis keep*/; // end of packet
assign sop = s_axi_tuser[0];
assign eop = s_axi_tlast && (y_cnt == VIDEO_H-1);
...
*/
.iR (s_axi_tdata[23:16]),
.iG (s_axi_tdata[15:8]),
.iB (s_axi_tdata[7:0]),
.VS (eop && (sop_data_cnt >= DATA_LEN) && s_axi_tvalid),
...
.ACTIV_C (focus_window_area & s_axi_tvalid) ,    // focus-window area
.ACTIV_V (s_axi_tvalid) , // full-screen
...
I2C_VCM_Config vcm_i2c(	
				...
				.iRST_N(~(eop && (sop_data_cnt >= DATA_LEN))), // trigger
				.VCM_DATA(VCM_DATA),
				.END(),//vcm_i2c_end
